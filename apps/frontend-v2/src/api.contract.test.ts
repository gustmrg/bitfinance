...
describe("backend endpoint contract", () => {
  it("covers every documented route with the right method and request shape", async () => {
    const seen: string[] = [];
    ...
    server.use(
      ...
      http.post(`${base}/organizations/join`, ({ request }) => { record(request.method, new URL(request.url).pathname); expect(new URL(request.url).searchParams.get("token")).toBe("token with spaces"); return HttpResponse.json(me.organizations[0]); }),
    ...
    await organizationsService.listAsync(); await organizationsService.getAsync(ids.organization); await organizationsService.createAsync("New org"); await organizationsService.updateAsync(ids.organization, "Updated org"); await organizationsService.getBudgetAsync(ids.organization); await organizationsService.upsertBudgetAsync(ids.organization, 5200); await organizationsService.createInviteAsync(ids.organization, "invitee@example.com", "Admin"); await organizationsService.joinAsync("token with spaces");
