...
export type FileCategory = "Invoice" | "Receipt" | "Boleto" | "Contract" | "Other";
export interface BillDocument { id: string; fileName: string; contentType: string; fileCategory: string; attachmentType: string }
export interface Bill { id: string; description: string; category: BillCategory; status: BillStatus; amountDue: number; amountPaid: number | null; dueDate: string; paymentDate: string | null; billSeriesId: string | null; occurrenceNumber: number | null; totalOccurrences: number | null; billSeriesType: BillSeriesType | null; billSeriesIsActive: boolean; documents: BillDocument[] }
