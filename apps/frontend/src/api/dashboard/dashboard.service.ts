...
const params = (filters?: DateFilters) => ({
...
export const dashboardService = {
  async getSummaryAsync(organizationId: string, filters?: DateFilters) {
    try {
      return (
        await authApi.get<DashboardSummary>(`/organizations/${organizationId}/dashboard/summary`, {
          params: params(filters),
        })
      ).data;
    } catch (error) {
      throw normalizeApiError(error, "api.dashboard.summary");
    }
  },
  async getUpcomingBillsAsync(
    organizationId: string,
    filters?: DateFilters,
  ): Promise<DashboardBill[]> {
    try {
      const data = (
        await authApi.get<{ data: DashboardBill[] }>(
      ...
      throw normalizeApiError(error, "api.dashboard.upcoming");
    }
  },
  async getRecentExpensesAsync(
    organizationId: string,
    filters?: DateFilters,
  ): Promise<DashboardExpense[]> {
    try {
      const data = (
        await authApi.get<{ data: DashboardExpense[] }>(
          `/organizations/${organizationId}/dashboard/recent-expenses`,
          { params: params(filters) },
        )
      ).data.data;
      return data.map((item) => ({ ...item, category: lower(item.category) }));
    } catch (error) {
      throw normalizeApiError(error, "api.dashboard.recent");
    }
  },
};
