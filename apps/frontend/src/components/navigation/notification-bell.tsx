...
function NotificationCopy({ notification }: { notification: AppNotification }) {
  ...
  return (
    <span className="notification-item__copy">
      <strong>{t(`notifications.${key}.title`)}</strong>
      <small>{t(`notifications.${key}.body`, { ...notification.parameters })}</small>
      ...
      >
        ...
        {(unread.data ?? 0) > 0 && (
          <span
            ...
            aria-label={t("notifications.unreadCount", { count: unread.data })}
          >
        ...
        >
          <header className="notification-panel__header">
            <span>
              ...
              <small>{t("notifications.currentOrganization")}</small>
            </span>
            {(unread.data ?? 0) > 0 && (
              <button
                type="button"
          ...
          <div className="notification-panel__list">
            {notifications.isPending && (
              <p className="notification-panel__state">{t("common.loading")}</p>
            )}
            ...
            {!notifications.isPending && !notifications.isError && items.length === 0 && (
              <p className="notification-panel__state">{t("notifications.empty")}</p>
            )}
            {items.map((notification) => {
              ...
              return (
                <Link
                  ...
                  onClick={() => {
                    setOpen(false);
                    if (!notification.readAt) mutations.markRead.mutate(notification.id);
                  }}
                >
