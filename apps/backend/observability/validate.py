...
def main():
    ...
    alerts_by_uid = {rule['uid']: rule for rule in alerts}
    delivery_failure_expr = alerts_by_uid['bf-delivery-failures']['data'][0]['model']['expr']
    assert 'stage="delivery"' in delivery_failure_expr
    assert 'rescheduled|terminal_failure' in delivery_failure_expr
    assert 'bitfinance_notification_delivery_oldest_age_seconds' in \
        alerts_by_uid['bf-delivery-age']['data'][0]['model']['expr']
    ...
    local_config = json.loads(run('docker', 'compose', '--env-file', str(HERE.parent / '.env.example'),
        '-f', str(HERE.parent / 'docker-compose.yml'), '-f', str(HERE.parent / 'docker-compose.override.yml'),
        '-f', str(HERE.parent / 'docker-compose.observability.yml'), '--profile', 'observability',
        'config', '--format', 'json', env=dict(env, GRAFANA_CLOUD_OTLP_ENDPOINT='https://example.test/otlp',
            GRAFANA_CLOUD_INSTANCE_ID='test', GRAFANA_CLOUD_API_KEY='test', OBSERVABILITY_ENABLED='true')))
    local_services = local_config['services']
    for name in ['bitfinance-api', 'bitfinance-mcp-server']:
        assert local_services[name]['environment']['Observability__Enabled'] == 'true'
        assert local_services[name]['environment']['OTEL_EXPORTER_OTLP_ENDPOINT'] == 'http://bitfinance-alloy:4317'
    assert {port['target'] for port in local_services['bitfinance-alloy']['ports']} == {12345, 4317, 4318}
    assert all(port['host_ip'] == '127.0.0.1' for port in local_services['bitfinance-alloy']['ports'])
...
''')
        ...
        (temp / 'curl').write_text('#!/usr/bin/env bash\nexit "${TEST_ALLOY_READY_FAILURE:-0}"\n')
        (temp / 'curl').chmod(0o755)
        (temp / 'sleep').write_text('#!/usr/bin/env bash\nexit 0\n')
        (temp / 'sleep').chmod(0o755)
        ...
        calls = temp / 'calls'
        settings = dict(env, PATH=f'{temp}:' + env['PATH'], IMAGE_TAG='test', MCP_IMAGE_TAG='test',
                        TEST_COMMANDS=str(calls), TEST_ALLOY_FAILURE='0')
        run('bash', str(HERE / 'deploy.sh'), 'backend', env=settings, cwd=temp)
        first_deploy = calls.read_text()
        assert 'validate /etc/alloy/config.alloy' in first_deploy
        assert '--force-recreate bitfinance-alloy' in first_deploy
        calls.write_text('')
        run('bash', str(HERE / 'deploy.sh'), 'backend', env=settings, cwd=temp)
        assert '--force-recreate bitfinance-alloy' not in calls.read_text()
        (temp / 'observability/alloy/config.alloy').write_text('changed')
        calls.write_text('')
        run('bash', str(HERE / 'deploy.sh'), 'backend', env=settings, cwd=temp)
        assert '--force-recreate bitfinance-alloy' in calls.read_text()
        deployed_hash = (temp / 'observability/alloy/.deployed-config.sha256').read_text()
        (temp / 'observability/alloy/config.alloy').write_text('not-ready')
        calls.write_text('')
        run('bash', str(HERE / 'deploy.sh'), 'backend',
            env=dict(settings, TEST_ALLOY_READY_FAILURE='1'), cwd=temp)
        assert (temp / 'observability/alloy/.deployed-config.sha256').read_text() == deployed_hash
